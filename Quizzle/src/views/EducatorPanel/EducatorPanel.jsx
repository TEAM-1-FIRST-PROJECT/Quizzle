  ...
  return (
    <div className="bg-black min-h-screen p-5">
      <div className=" pt-16 pb-24 ml-[320px] mr-[34px] mb-[80px] mt-[110px] justify-center items-center border-4 p-10 rounded-lg bg-gradient-to-bl from-indigo-400 to-cyan-400">
        ...
        />
        <button
          className="mt-2 px-4 py-2 text-sm font-medium text-white bg-cyan-400 rounded-md hover:bg-green-400 float-right transform transition duration-500 ease-in-out hover:scale-105"
          onClick={() => {
            setFinishedQuizzes(!finishedQuizzes);
          }}
        >
          {finishedQuizzes
            ? "switch to finished quizzes"
            : "switch to opened quizzes"}
        </button>
          ...
          <tbody>
            {openQuizzes.map((quiz) => (
              <tr key={quiz.id}>
                ...
                <td className="border px-4 py-2">
                  {quiz.assignedUsers
                    ? Object.keys(quiz.assignedUsers).length
                    : 0}
                </td>
                <td className="border px-4 py-2">
                  <a
                    href={`/assign-quiz/${quiz?.id}`}
                    ...
                    onClick={() => {
                      history.push(`/assign-quiz/${quiz?.id}`);
                    }}
                  >
                    Assign
                  </a>
